...
cap_cv* get_capture_video_stream(const char *path) {
    ...
    try {
        //path = "v4l2src device=/dev/video0 ! image/jpeg, width=1920, height=1080, framerate=30/1 ! jpegparse ! jpegdec ! videoconvert ! appsink";
        ...
        //std::string pipeline = "v4l2src device=/dev/video0 ! image/jpeg, width=(int)1920, height=(int)1080, framerate=15/1 ! jpegparse ! jpegdec ! videoconvert ! appsink sync=false";

        std::string pipeline = path;

        //std::string pipeline = "v4l2src io-mode=2 device=/dev/video1 ! video/x-h264,width=1920,height=1080,framerate=15/1,streamformat=(string)byte-stream ! omxh264dec  !  videoconvert ! appsink sync=false ";

        std::cout << "Using pipeline: \n\t" << pipeline << "\n";

        cap = new cv::VideoCapture(pipeline, cv::CAP_GSTREAMER);

        //cap->set(CV_CAP_PROP_FRAME_WIDTH, 1920); //samson
        //cap->set(CV_CAP_PROP_FRAME_HEIGHT, 1080); //samson     
        //cap->set(CV_CAP_PROP_FPS, 15); //samson    
...
cap_cv* get_capture_webcam(int index)
{
    ...
    try {
		...
		cap->set(CV_CAP_PROP_FOURCC, CV_FOURCC('M', 'J', 'P', 'G') ); //samson 
		//cap->set(CV_CAP_PROP_FOURCC, CV_FOURCC('I', '4', '2', '0') ); //samson 
		cap->set(CV_CAP_PROP_FRAME_WIDTH, 1920); //samson
		cap->set(CV_CAP_PROP_FRAME_HEIGHT, 1080); //samson 	
		//cap->set(CV_CAP_PROP_FPS, 5); //samson 
		cap->set(CV_CAP_PROP_FPS, 30); //samson 
...
void save_mat_jpg(cv::Mat img_src, const char *name)
{
	...
	//samson
	stbi_write_jpg(name, img_rgb.cols, img_rgb.rows, 3, (char *)img_rgb.data, 20);
